...
const DiscordWebhooksContainer = document.querySelector(
  "#DiscordWebhooks"
) as HTMLDivElement;

function showDiscordWebhooks(discordWebhooks: string[]) {
...
function addDiscordWebhook(index: number, discordWebhook?: string) {
  ...
  const webhookHTML = `
    <div>
      <input
        ...
        data-discordWebhookIndex=${index} ${
    !!discordWebhook ? "readonly" : ""
  ...
  webhookElement.querySelector("button").addEventListener("click", () => {
    ...
    else webhookElement.querySelector("input").value = "";
  ...
  webhookElement.querySelector("input").addEventListener("input", (e) => {
    chrome.storage.sync.get(["discordWebhooks"], (result) => {
      ...
      if (!newWebhook.includes("discord")) return;
