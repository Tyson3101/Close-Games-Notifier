...
// Show Schedule
const gamesContainer = document.querySelector(".games") as HTMLDivElement;
const hideScoresCheckbox = document.querySelector(
  "#hideScores"
) as HTMLInputElement;
...
async function getGamesForScoreboard() {
  ...
  const games = (await data.json()).scoreboard.games as Game[];
...
async function createScoreboardGame(game: Game) {
  ...
  <div class="scoreboard" id="${gameId}">
    <div class="teams">
      ...
      <div class="score">
        <span data-homeScore ${
          !hideScores ? "" : `class="hidden"`
        }>${homeTeamScore}</span><span data-hiddenScore ${
    hideScores ? "" : `class="hidden"`
  }>--</span> |
        <span data-awayScore ${
          !hideScores ? "" : `class="hidden"`
        }>${awayTeamScore}</span><span data-hiddenScore ${
    hideScores ? "" : `class="hidden"`
  }>--</span>
      </div>
      <div>
...
`;
  ...
  function changeMuteStatus() {
    const muted = scoreboard.querySelector(
      "[data-muteStatus]"
