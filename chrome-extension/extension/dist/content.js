...
chrome.runtime.onMessage.addListener(async ({ type, game }, _, sendResponse) => {
    ...
    async function UIChangeMuteStatus() {
        let bellElement = popupContainer.querySelector("#notiCloseGameBell img");
        let buttonElement = popupContainer.querySelector("#ChangeMutedStatusBtn");
        let newMutedStatus = !(await getMutedStatus(game.gameId));
        bellElement.src = newMutedStatus
            ? unmutedBellImage.src
            : mutedBellImage.src;
        bellElement.title = `${newMutedStatus ? "Mute" : "Unmute"} Notifications from this Game`;
        buttonElement.innerText = `${newMutedStatus ? "Mute" : "Unmute"} Game`;
    }
    return true;
});
function getMutedStatus(gameId) {
...
function StorageChangeMutedStatus(gameId) {
    chrome.storage.sync.get(["mutedGames"], (result) => {
        const mutedGames = result.mutedGames;
        if (mutedGames && mutedGames[gameId]) {
